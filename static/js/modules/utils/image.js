...
class ImageCache {
	...
	initCache() {
		try {
			...
			if (cached) {
				...
				console.log(`Restored ${this.cache.size} cached images from localStorage`);
			}
		} catch (error) {
	...
	async preloadImages(srcs) {
		const promises = srcs.map(src => this.preloadImage(src).catch(err => {
