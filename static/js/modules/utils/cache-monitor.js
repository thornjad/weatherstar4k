...
class CacheMonitor {
	 ...
	 */
	startMonitoring(intervalMs = 30000) { // Default: log every 30 seconds
		if (this.monitoring) {
	...
	getPerformanceSummary() {
		...
		const avgHitRate = recent.reduce((sum, entry) => {
	...
	async testPersistence(testImages) {
		...
		const cachedCount = testImages.filter(img => imageCache.isCached(img)).length;
		
		console.log(`    Persistence check: ${cachedCount}/${testImages.length} images still cached`);
	...
	async run() {
		...
		console.log('✅ Cache monitor is now active. Reload the page to stop monitoring.');
	}
}
