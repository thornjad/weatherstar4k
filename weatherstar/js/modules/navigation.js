...
const message = data => {
  ...
  if (data.type === 'navButton') {
    return handleNavButton(data.message);
  }
...
const updateStatus = value => {
  ...
  if (displays[0].status === STATUS.loading) {
    return;
  }
...
const countLoadedDisplays = () =>
  displays.reduce((acc, display) => {
    if (display?.showOnProgress === false) {
      return acc;
    }
    if (display?.status !== STATUS.loading) {
      return acc + 1;
    }
    return acc;
  }, 0);
...
const displayNavMessage = myMessage => {
  if (myMessage.type === msg.response.previous) {
    loadDisplay(-1);
  }
  if (myMessage.type === msg.response.next) {
    loadDisplay(1);
  }
};
...
const navTo = direction => {
  ...
  if (!current) {
    ...
    let displayCount = 0;
    do {
      if (displays[displayCount]?.status === STATUS.loaded && displays[displayCount]?.timing?.totalScreens > 0) {
        firstDisplay = displays[displayCount];
      }
      displayCount += 1;
  ...
  if (direction === msg.command.nextFrame) {
    currentDisplay().navNext();
  }
  if (direction === msg.command.previousFrame) {
    currentDisplay().navPrev();
  }
};
...
const loadDisplay = direction => {
  ...
  if (direction < 0) {
    newDisplay.showCanvas(msg.command.lastFrame);
  }
  if (direction > 0) {
    newDisplay.showCanvas(msg.command.firstFrame);
  }
};
...
const registerDisplay = display => {
  if (displays[display.navId]) {
    console.warn(`Display nav ID ${display.navId} already in use`);
  }
