...
class Almanac extends WeatherDisplay {
  ...
  calcSunMoonData(weatherParameters) {
    ...
    do {
      ...
      // check for 4 cases
      if (lastPhase < 0.25 && phase >= 0.25) {
        moon.push(this.getMoonTransition(0.25, 'First', moonDate));
      }
      if (lastPhase < 0.5 && phase >= 0.5) {
        moon.push(this.getMoonTransition(0.5, 'Full', moonDate));
      }
      if (lastPhase < 0.75 && phase >= 0.75) {
        moon.push(this.getMoonTransition(0.75, 'Last', moonDate));
      }
      if (lastPhase > phase) {
        moon.push(this.getMoonTransition(0.0, 'New', moonDate));
      }
  ...
  getMoonTransition(threshold, phaseName, start, iteration = 0) {
    ...
    // decreasing test
    if (iteration % 2 === 0) {
      test = (lastPhase, testPhase) => lastPhase > threshold && testPhase <= threshold;
    }
  ...
  async drawCanvas() {
    ...
    const days = info.moon.map(MoonPhase => {
      ...
      const date = MoonPhase.date.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      });
