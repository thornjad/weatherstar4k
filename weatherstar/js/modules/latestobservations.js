  ...
  async drawCanvas() {
    ...
    const lines = sortedConditions.map(condition => {
      ...
      if (WindSpeed > 0) {
        fill.wind =
          windDirection +
          Array(6 - windDirection.length - WindSpeed.toString().length).join(' ') +
          WindSpeed.toString();
...
const getStations = async stations => {
  const stationData = await Promise.all(
    stations.map(async station => {
      try {
        const data = await fetchAsync(`https://api.weather.gov/stations/${station.id}/observations/latest`, 'json', {
          retryCount: 1,
        ...
        if (
          ...
          data.properties.windSpeed.value === null
        ) {
          return false;
        }
