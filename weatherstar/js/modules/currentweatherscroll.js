...
const drawScreen = async () => {
  ...
  elemForEach('.weather-display .scroll', elem => {
    ...
    if (elem.parentElement.id === 'progress-html') {
      return;
    }
...
const screens = [
  ...
  // wind
  data => {
    let text = data.WindSpeed > 0 ? `Wind: ${data.WindDirection} ${data.WindSpeed} ${data.WindUnit}` : 'Wind: Calm';
